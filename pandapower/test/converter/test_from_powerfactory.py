...
@pytest.mark.skipif(not PF_INSTALLED, reason='powerfactory must be installed')
def test_pf_symmetrical_tap_changer():
    app = pf.GetApplication()
    # import the tap changer test grid to powerfactory
    path = os.path.join(pp_dir, 'test', 'converter', 'testfiles', 'test_tap_symmetrical.pfd')
    prj = import_project(path, app, 'test_tap_chg_symmetrical', import_folder='TEST_IMPORT', clear_import_folder=True)
    prj_name = prj.GetFullName()
